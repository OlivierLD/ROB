 ...
 * For the main: Input is taken from a text file, provided at runtime.
 * Start from the cones, and find the position.
 * GHA and Decl are calculated in the main, for the given time.
 * Observer's position is found at the end of the computeCones method.
 */
public class ConesSolver {
    ...
    public static class BodyData {
        ...
        public BodyData(String bodyName, String date, Double gha, Double decl, Double obsAlt) {
    ...
    public static BodyData computeBodyData(String body, String dateDuration, double obsAlt) throws Exception {
        ...
        bodyData.setDate(DURATION_FMT.format(date.getTime()));
...
//        }
    ...
    public static GeoPoint computeCones(List<BodyData> originalListBodyData, boolean verbose) throws Exception {

        // Compute GHA and Decl
        List<BodyData> listBodyData = new ArrayList<>();
        originalListBodyData.forEach(bd -> {
            try {
                BodyData bodyData = computeBodyData(bd.getBodyName(), bd.getDate(), bd.getObsAlt());
    ...
    public static void main(String... args) {
        ...
        List<BodyData> listBodyData = new ArrayList<>(); // The one to send to computeCones
...
//                                            }
                            ...
                            if (body.get() == null || date.get() == null || alt.get() == null) {
                                ...
                                try {
                                    ...
                                    // listBodyData.add(bodyData);
                                    listBodyData.add(new BodyData(body.get(), date.get(), null, null, alt.get()));
                                } catch (Exception ex) {
                                    // ex.printStackTrace();
                                    throw new RuntimeException(ex);
                                }
...
//                                                    bd.getBodyName(),
//                                                    GeomUtil.decToSex(bd.getObsAlt(), GeomUtil.SHELL, GeomUtil.NONE), bd.getObsAlt(),
//                                                    GeomUtil.decToSex(bd.getGha(), GeomUtil.SHELL, GeomUtil.NONE), bd.getGha(),
//                                                    GeomUtil.decToSex(bd.getDecl(), GeomUtil.SHELL, GeomUtil.NS), bd.getDecl()));

        listBodyData.forEach(bd-> System.out.printf("%s:\t ObsAlt: %s (%f)\n",
                bd.getBodyName(),
                GeomUtil.decToSex(bd.getObsAlt(), GeomUtil.SHELL, GeomUtil.NONE), bd.getObsAlt()));

        String content;
        try {
            content = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(listBodyData);
            System.out.printf("Invoking computeCones with List of Body Data:\n%s\n", content);
        ...
        try {
            // That one can be called from a REST server
            GeoPoint gp = computeCones(listBodyData, false);
            System.out.println("--------------------------------");
            System.out.printf("Final calculated intersection at %s\n", gp);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
