	...
	private Response getSunDataNow(Request request) {
		...
		if (request.getContent() != null && request.getContent().length > 0) {
			...
			if (!"null".equals(payload)) {
				...
				try {
					PosAndDate pad = mapper.readValue(stringReader, PosAndDate.class);
...
//						System.out.println("   >> Turned to Calendar:" + refDate.getTime());
					...
					System.err.println("--- getSunDataNow ---");
					ex.printStackTrace();
					System.err.println("---------------------");
...
//					System.out.println("getSunDataNow >> UTC Date:" + utcDate);
	...
	private Response getConesIntersections(Request request) {
		...
		if (false) {
			try {
				ConeInput coneInput1 = new ConeInput("Body 1", 123.45, 23.45, 34.56);
				ConeInput coneInput2 = new ConeInput("Body 2", 234.56, 34.56, 45.67);
				List<ConeInput> coneList = Arrays.asList(new ConeInput[] { coneInput1, coneInput2 });
				String duh = mapper.writeValueAsString(coneList);
				System.out.printf("conesList (payload) : [%s]\n", duh);
			} catch (Exception ex) {
				ex.printStackTrace();
			}
		}

		ConeInput[] conesInput = null;

		if (request.getContent() != null && request.getContent().length > 0) {
			String payload = new String(request.getContent());
			if ("true".equals(System.getProperty("rest.mps.verbose"))) {
				this.MPSRequestManager.getLogger().log(Level.INFO, String.format(">> getConesIntersections with payload %s", payload));
			}
			if (!"null".equals(payload)) {
				StringReader stringReader = new StringReader(payload);
				try {
		...
		try {
			...
			double zStep = 1.0d; // 0.1d; // 1 may be too small..
			int nbLoops  = 5; // 4 should be good enough
			intersections = MPSToolBox.resolve2Cones(new Date(), // Date not really used...
					conesInput[0].getObsAlt(), conesInput[0].getGHA(), conesInput[0].getD(),
					new Date(), // Date not really used...
					conesInput[1].getObsAlt(), conesInput[1].getGHA(), conesInput[1].getD(),
					zStep, nbLoops, false, false);
		} catch (Exception ex) {
			response = HTTPServer.buildErrorResponse(response,
					Response.BAD_REQUEST,
					new HTTPServer.ErrorPayload()
	...
	private Response processConesIntersections(Request request) {
		...
		try {
			...
			response = HTTPServer.buildErrorResponse(response,
					...
					new HTTPServer.ErrorPayload()
							.errorCode("MPS-0016")
							.errorMessage(String.format("MPSToolBox.processIntersectionsList: %s", ex.toString())));
			return response;
		}
	...
	private Response getSunPathInTheSky(Request request) {
		...
		if (request.getContent() != null && request.getContent().length > 0) {
			...
			if (!"null".equals(payload)) {
				...
				try {
					pas = mapper.readValue(stringReader, PosAndStep.class);
					...
					System.err.println("--- getSunPathInTheSky ---");
					ex.printStackTrace();
					System.err.println("--------------------------");
					response = HTTPServer.buildErrorResponse(response,
							Response.BAD_REQUEST,
	...
	private Response getSunDataBetween(Request request) {
		...
		if (request.getContent() != null && request.getContent().length > 0) {
			...
			if (!"null".equals(payload)) {
				StringReader stringReader = new StringReader(payload);
				try {
					pos = mapper.readValue(stringReader, GeoPoint.class);
				} catch (Exception ex) {
					response = HTTPServer.buildErrorResponse(response,
							Response.BAD_REQUEST,
							new HTTPServer.ErrorPayload()
	...
	private Response getBodyDeclination(Request request) {
		...
		if (request.getContent() != null && request.getContent().length > 0) {
			...
			if (!"null".equals(payload)) {
				...
				StringReader stringReader = new StringReader(payload);
				try {
					DeclinationOptions options = mapper.readValue(stringReader, DeclinationOptions.class);
...
//					System.out.println(String.format("Calculating between %s and %s", UTC_FMT.format(fromCal.getTime()), UTC_FMT.format(toCal.getTime())));
					...
					String content;
					try {
						content = mapper.writeValueAsString(declHolder);
					} catch (JsonProcessingException jpe) {
						response = HTTPServer.buildErrorResponse(response,
								Response.BAD_REQUEST,
								new HTTPServer.ErrorPayload()
		...
		if (request.getContent() != null && request.getContent().length > 0) {
			...
			if (!"null".equals(payload)) {
				try {
					...
					try {
						options = mapper.readValue(stringReader, PerpetualAlmanacOptions.class);
	...
	public static class CelestialBodyData {
		...
		public double getInstrumentalAltitude() {
			return instrumentalAltitude;
		}
	...
	public static class SightReductionData {
		...
		public AstroComputerV2.Pos getEstimatedPosition() {
		...
		public CelestialBodyData getCbd() {
			return cbd;
		}
	}
	...
	private Response sightReductionImplementation(Response response, String payload, boolean reverse) {
		...
		if (!"null".equals(payload)) {
			StringReader stringReader = new StringReader(payload);
			try {
				SightReductionData userData = mapper.readValue(stringReader, SightReductionData.class);
...
//								lunar = Context.starMoonDist;
						...
						String content;
						try {
							content = mapper.writeValueAsString(reduced);
						} catch (JsonProcessingException jpe) {
							response = HTTPServer.buildErrorResponse(response,
									Response.BAD_REQUEST,
									new HTTPServer.ErrorPayload()
											.errorCode("NAV-0004-1")
											.errorMessage(jpe.toString())
											.errorStack(HTTPServer.dumpException(jpe)));
							return response;
						}
...
//					return response;
	...
	public static class PositionsInTheSky {
		...
		public AstroComputerV2.GP getSun() {
			return sun;
		}

		public AstroComputerV2.GP getMoon() {
			return moon;
		}
		...
		public List<AstroComputerV2.GreatCircleWayPointWithBodyFromPos> getMoonToSunSkyRoute() {
			return moonToSunSkyRoute;
		}

		public double getMoonTilt() {
			return moonTilt;
		}

		public List<AstroComputerV2.GP> getWanderingBodies() {
			return wanderingBodies;
		}
		...
		public AstroComputerV2.Pos getFrom() {
			return from;
		}

		public AstroComputerV2.OBS getSunObs() {
			return sunObs;
		}

		public AstroComputerV2.OBS getMoonObs() {
			return moonObs;
		}

		public AstroComputerV2.OBS getAriesObs() {
			return ariesObs;
		}

		public FmtDate gettPass() {
			return tPass;
		}

		public FmtDate getSolarDate() {
			return solarDate;
		}
	}
	...
	public static class SunMoonDecAlt {
		...
		public double getSunDecl() {
			return sunDecl;
		}

		public double getMoonDecl() {
			return moonDecl;
		}

		public double getSunAlt() {
			return sunAlt;
		}

		public double getMoonAlt() {
			return moonAlt;
		}
