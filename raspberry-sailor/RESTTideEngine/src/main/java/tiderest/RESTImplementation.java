...
public class RESTImplementation {
	...
	private Response getWaterHeightPlus(Request request) {
	 ...
	 */
	private Response getWaterHeightWithDetails(Request request, boolean withDetails) {
			...
			if (proceed) {
				...
				if (request.getContent() != null && request.getContent().length > 0) {
					...
					if (!"null".equals(payload)) {
						...
						try {
							WaterHeightOptions options = mapper.readValue(stringReader, WaterHeightOptions.class);
							if (options.step == 0 &&
	...
	//								map.put(now.getTime().toString(), wh * unitSwitcher(ts, unitToUse));
									...
									harmonics.stream()
											.forEach(harmonicCoeff -> {
												...
												while (_now.before(_calTo) || _now.equals(_calTo)) {
													...
													Date jan1st = new GregorianCalendar(year, Calendar.JANUARY, 1).getTime();
								...
								String content;
								try {
									content = mapper.writeValueAsString(tideTable);
								} catch (JsonProcessingException jpe) {
									System.err.println("--- getWaterHeightWithDetails ---");
									jpe.printStackTrace();
									System.err.println("---------------------------------");
									response = HTTPServer.buildErrorResponse(response,
											Response.BAD_REQUEST,
...
//			System.out.println("Pattern:" + pattern.toString());
	 ...
	 * @return Response
	 */
	private Response publishMoonCalendar(Request request) {
	 ...
	 * @return Response
	 */
	private Response publishTideTable(Request request) {
	...
	private Response publishTideDocument(Request request, String script) {
		Response response = new Response(request.getProtocol(), Response.STATUS_OK);
		List<String> prmValues = request.getPathParameters(); // RESTProcessorUtil.getPathPrmValues(request.getRequestPattern(), request.getPath());
		String stationFullName = "";
		if (prmValues.size() == 1) {
		...
		if (request.getContent() != null && request.getContent().length > 0) {
			...
			if (!"null".equals(payload)) {
				...
				try {
					options = mapper.readValue(stringReader, PublishingOptions.class);
...
//				String unescaped = URLDecoder.decode(stationFullName, StandardCharsets.UTF_8.toString());
	...
	private Response publishSunDocument(Request request) {
		return publishSunDocument(request, TidePublisher.AGENDA_TABLE); // TODO Tweak the stylesheet...
	}
	private Response publishSunDocument(Request request, String script) {
		Response response = new Response(request.getProtocol(), Response.STATUS_OK);
		List<String> prmValues = request.getPathParameters(); // RESTProcessorUtil.getPathPrmValues(request.getRequestPattern(), request.getPath());
		if (request.getContent() != null && request.getContent().length > 0) {
			String payload = new String(request.getContent());
			if (!"null".equals(payload)) {
				...
				if (!errMess.trim().isEmpty()) {
					...
					try {
						String toJson = mapper.writeValueAsString(options);
	...
	private static class TideTable {
		...
		public String getToPrm() {
			return toPrm;
		}

		public List<TideUtilities.TimedValue> getTable() {
			return table;
		}

		public Map<String, WhDate> getHeights() {
			return heights;
		}
	...
	private static class WaterHeightOptions {
		...
		public RESTImplementation.unit getUnit() {
			return unit;
		}
	}
