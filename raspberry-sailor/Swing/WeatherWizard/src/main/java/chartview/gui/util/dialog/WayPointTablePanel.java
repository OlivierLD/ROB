...
public final class WayPointTablePanel
    ...
    private final JButton removeButton = new JButton(WWGnlUtilities.buildMessage("remove"));
    private final JButton upButton = new JButton(new ImageIcon(this.getClass().getResource("up.png")));
    private final JButton downButton = new JButton(new ImageIcon(this.getClass().getResource("down.png")));

    private final String colName = "Way Point(s)";
    private List<GeoPoint> aliwp = null;

    private final String[] names = {colName};

    private transient TableModel dataModel;

    private GeoPoint[][] data = new GeoPoint[0][0];
...
//          System.out.println("Moving row [" + selectedRow + "] down");
    ...
    public List<GeoPoint> getData() {
        List<GeoPoint> al = new ArrayList<>(data.length);
        for (GeoPoint[] datum : data) {
          al.add(datum[0]);
        }
        return al;
    }
