        ...
        try {
            ...
            synchronized (parser) {
                ...
                for (int i = 0; i < marks.getLength(); i++) {
                    ...
                    addLineInTable(name, new Latitude(latitude), new Longitude(longitude), Boolean.valueOf(show));
                }
                fis.close();
            }
        } catch (Exception e) {
...
//    sorter.checkModel();
    ...
    void addButton_actionPerformed(ActionEvent e) {
        try {
            addLineInTable("", new Latitude("0 0.0 N"), new Longitude("0 0.0 E"));
    ...
    public void saveData() {
        ...
        for (Object[] datum : data) {
          ...
          mark.setAttribute("name", (String) datum[0]);
          mark.setAttribute("show", ((Boolean) datum[3]).toString());
          Element latitude = doc.createElement("latitude");
          latitude.setAttribute("deg", Integer.toString(((Latitude) datum[1]).deg));
          latitude.setAttribute("min", Double.toString(((Latitude) datum[1]).min));
          latitude.setAttribute("sign", ((Latitude) datum[1]).nsew);
          mark.appendChild(latitude);

          Element longitude = doc.createElement("longitude");
          longitude.setAttribute("deg", Integer.toString(((Longitude) datum[2]).deg));
          longitude.setAttribute("min", Double.toString(((Longitude) datum[2]).min));
          longitude.setAttribute("sign", ((Longitude) datum[2]).nsew);
