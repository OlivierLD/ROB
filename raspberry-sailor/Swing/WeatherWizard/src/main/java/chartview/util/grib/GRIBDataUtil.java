...
public class GRIBDataUtil {
    ...
    public static double[] getCurrentSpeedBoundaries(GribHelper.GribConditionData gribData) {
        double minValue = Double.MAX_VALUE;
        double maxValue = Double.MIN_VALUE;
        for (int h = 0; gribData.getGribPointData() != null && h < gribData.getGribPointData().length; h++) {
            for (int w = 0; w < gribData.getGribPointData()[h].length; w++) {
    ...
    public static double getGRIBWindSpeed(float x, float y) {
    ...
    public static double[] get500MbBoundaries(GribHelper.GribConditionData gribData) {
        double minValue = Double.MAX_VALUE;
        double maxValue = Double.MIN_VALUE;
        double prevVal = 0.0;
        for (int h = 0; gribData.getGribPointData() != null && h < gribData.getGribPointData().length; h++) {
    ...
    public static double[] getAirTempBoundaries(GribHelper.GribConditionData gribData) {
        double minValue = Double.MAX_VALUE;
        double maxValue = Double.MIN_VALUE;
        for (int h = 0; gribData.getGribPointData() != null && h < gribData.getGribPointData().length; h++) {
            for (int w = 0; w < gribData.getGribPointData()[h].length; w++) {
    ...
    public static double[] getWaveHgtBoundaries(GribHelper.GribConditionData gribData) {
        double minValue = Double.MAX_VALUE;
        double maxValue = Double.MIN_VALUE;
        for (int h = 0; gribData.getGribPointData() != null && h < gribData.getGribPointData().length; h++) {
            for (int w = 0; w < gribData.getGribPointData()[h].length; w++) {
    ...
    public static double[] getRainBoundaries(GribHelper.GribConditionData gribData) {
        double minValue = Double.MAX_VALUE;
        double maxValue = Double.MIN_VALUE;
        for (int h = 0; gribData.getGribPointData() != null && h < gribData.getGribPointData().length; h++) {
            for (int w = 0; w < gribData.getGribPointData()[h].length; w++) {
...
//  System.out.println("Rain boundaries:" + minValue + "/" + maxValue);
    ...
    public static List<List<List<GeoPoint>>> generateIsoTWS(GribHelper.GribConditionData gribData, int[] va) {
        return generateIsoCurves(gribData, va, TYPE_TWS);
    }

    public static List<List<List<GeoPoint>>> generateIsobars(GribHelper.GribConditionData gribData, int[] va) {
        return generateIsoCurves(gribData, va, TYPE_PRMSL);
    }

    public static List<List<List<GeoPoint>>> generateIso500(GribHelper.GribConditionData gribData, int[] va) {
        return generateIsoCurves(gribData, va, TYPE_500MB);
    }

    public static List<List<List<GeoPoint>>> generateIsotherm(GribHelper.GribConditionData gribData, int[] va) {
        return generateIsoCurves(gribData, va, TYPE_TMP);
    }

    public static List<List<List<GeoPoint>>> generateIsowaves(GribHelper.GribConditionData gribData, int[] va) {
        return generateIsoCurves(gribData, va, TYPE_WAVE);
    }

    public static List<List<List<GeoPoint>>> generateIsorain(GribHelper.GribConditionData gribData, int[] va) {
    ...
    private static List<List<List<GeoPoint>>> generateIsoCurves(GribHelper.GribConditionData gribData, int[] va, int dataType) {
        double[] dva = new double[va.length];
        for (int i = 0; i < va.length; i++) {
          dva[i] = va[i];
        }
    ...
    private static List<List<List<GeoPoint>>> generateIsoCurves(GribHelper.GribConditionData gribData, double[] va, int dataType) {
        List<List<List<GeoPoint>>> alalalgp = new ArrayList<>();
        for (double v : va) {
          //  long before = System.currentTimeMillis();
          alalalgp.add(detect(v, gribData, dataType));
    ...
    private static List<List<GeoPoint>> detect(double val2detect, GribHelper.GribConditionData gribData, int dataType) {
        List<List<GeoPoint>> alalgp = new ArrayList<>();
        List<GeoPoint> iso = new ArrayList<>();
        ...
        while (proceed) {
            if (iso.size() == 0) {
              proceed = false;
                ...
                List<GeoPoint> is = new ArrayList<>();
                alalgp.add(is);
                is.add(p);
                iso.remove(p);
...
//                   ((a.getL() - b.getL()) * (a.getL() - b.getL())));
    ...
    public static boolean thereIsVariation(GribFile gribFile, int option) {
        boolean ok = false;
...
//          throw new RuntimeException("Type TWS Not supported in this method");
    ...
    public static boolean thereIsVariation(GribHelper.GribConditionData gribData, int option) {
        boolean ok = false;
        double prevVal = 0D;
        double value = 0D;
        boolean first = true;
    ...
    public static double[] generate3dFile(GribHelper.GribConditionData gribData,
                                          int option,
                                          double scaleFactor,
                                          double valueFactor,
...
//    gribData.wind = ok;
                        ...
                        if (option == TYPE_500MB) {
                          ...
                          if (value <= 0.0) {
                              if (enforceWackyPrmsl500mbTmpValues) {
                                  ...
                                  System.out.println("Argh ! 0 value for 500MB !");
                              }
                          }
                        } else if (option == TYPE_PRMSL) {
                            value = gribData.getGribPointData()[h][w].getPrmsl() / 10D;
                            if (value <= 0.0) {
                                if (enforceWackyPrmsl500mbTmpValues) {
                                    ...
                                    System.out.println("Argh ! 0 value for PRMSL !");
                                }
                            }
                        } else if (option == TYPE_TMP) {
                            value = gribData.getGribPointData()[h][w].getAirtmp();
                            if (value <= 0.0) {
                                if (enforceWackyPrmsl500mbTmpValues) {
                                    ...
                                    System.out.println("Argh ! 0 value for TMP !");
                                }
                            }
                        } else if (option == TYPE_RAIN) {
                          value = gribData.getGribPointData()[h][w].getRain() * 3_600D;
    ...
    public static void generateChart3dFile(GribHelper.GribConditionData gribData,
                                                 double scaleFactor,
                                                 ChartPanel chartPanel) {
        XMLDocument xmlDoc = null;
        XMLElement root = null;
...
//          System.out.println("Min:" + min + ", Max: " + max + ", Avg:" + (sum/(double)ptIdx));
    ...
    private final static int VALUE_FACTOR = 2_500;
    ...
    public static void generateKMLGRIBFile(GribHelper.GribConditionData gribData,
                                                 int[] options,
                                                 ChartPanel chartPanel,
