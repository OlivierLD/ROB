...
public class OneGRIBTablePanel
    ...
    private final JButton generateObjButton = new JButton();
        ...
        try {
            ...
            for (int h = 0; h < height; h++) {
                for (int w = 0; w < width; w++) {
                    bw.write("v " + Float.toString(((w * amplitude) - ((width * amplitude) / 2F)) / coordCoeff) + " " + Float.toString(((h * amplitude) - ((height - amplitude) / 2F)) / coordCoeff) + " " + Float.toString(((Float) data[h][w] - amplitude) / valueCoeff) + "\n");
                }
            }
            bw.write("# Vertices Textures\n");
            for (int h = 0; h < height; h++) {
                for (int w = 0; w < width; w++) {
                    bw.write("vt " + Float.toString(((w * amplitude) - ((width * amplitude) / 2F)) / coordCoeff) + " " + Float.toString(((h * amplitude) - ((height - amplitude) / 2F)) / coordCoeff) + " " + Float.toString(((Float) data[h][w] - amplitude) / valueCoeff) + "\n");
                }
            }
            bw.write("# Faces\n");
