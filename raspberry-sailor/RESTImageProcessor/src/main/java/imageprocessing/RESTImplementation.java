...
public class RESTImplementation {
	...
	private Response uploadImage(Request request) {
		...
		if (request.getContent() != null && request.getContent().length > 0) {
			...
			if (imageData != null) {
				System.out.printf("Image: %d bytes.\n", imageData.length);
				...
				try {
					...
					txRequests.forEach(json -> {
								try {
									...
									String jsonMapStr = mapper.writeValueAsString(json);
									// System.out.printf("Reading LinkedHashMap [%s]\n", jsonMapStr);
									PullTxManager.TxRequest txRequest = mapper.readValue(
//											((LinkedHashMap)json).toString(),
											jsonMapStr,
									...
									if (verbose) {
										System.out.printf("Done with %s... final document is %s\n", txRequest.getUrl(), txRequest.getReturned());
									}
					...
					String content;
					try {
						content = mapper.writeValueAsString(resultList);
					} catch (JsonProcessingException jpe) {
						response = HTTPServer.buildErrorResponse(response,
								Response.BAD_REQUEST,
								new HTTPServer.ErrorPayload()
