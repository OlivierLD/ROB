 ...
 *  <trk>
 *     <trkseg>
 *       <trkpt lat="49.293799500" lon="-0.095371167">
 *         <time>2023-06-18T08:25:03Z</time>
 *       </trkpt>
 *       <trkpt lat="49.293932667" lon="-0.095488667">
 *         <time>2023-06-18T08:31:32Z</time>
 *       </trkpt>
 *	...
 *
 *
 ...
 *
 * See CLI prms:
 * --file-name: Mandatory. GPX file name, on the file system
 * --output-file-name: Optional. Will be generated from --file-name if missing
 * Example:
 *   --file-name:./sample-data/patrick/Dives-LeHavre01.gpx
 *   --output-file-name:./sample-data/patrick/Dives-LeHavre01.json
 */
public class GPXtoJSONPosPlus {
	...
	private static void transform(String fileInName,
		...
		URL gpxURL = new File(fileInName).toURI().toURL();
		List<GPXUtil.DatedPoint> allGp = GPXUtil.parseGPXData(gpxURL, -1L, -1L);
		...
		allGp.stream().forEach(gp -> {
			ObjectToLog otl = new ObjectToLog(gp.getPt().getLatitude(), gp.getPt().getLongitude());
			otl.setRmcDate(SDF_UTC.format(gp.getDate()));
			// Finally
			jsonArray.add(otl);
		});
	...
	public static void main(String... args) {
		...
		try {
			...
			GPXtoJSONPosPlus.transform(inputFileName,
										archiveName,
