...
//	String findCommand = String.format("find %s -name '*.nmea'", System.getProperty("user.dir", "."));
	 ...
	 * @return The details!
	 */
	private HTTPServer.Response getLogFileDetails(HTTPServer.Request request) {
		HTTPServer.Response response = new HTTPServer.Response(request.getProtocol(), HTTPServer.Response.STATUS_OK);
		List<String> prmValues = request.getPathParameters();
		if (prmValues.size() != 1) {
			response.setStatus(HTTPServer.Response.BAD_REQUEST);
		...
		System.setProperty("summary", "true");
		System.setProperty("gencsv", "false");
		System.setProperty("swingGUI", "false");
		System.setProperty("speed.unit", "KN");

		try {
			File temp = File.createTempFile("details", ".txt");
			System.out.println("Temporary file created at: " + temp.getAbsolutePath());

			PrintStream fos = new PrintStream(temp);
			System.setOut(fos);

			LogAnalyzer.main(logFileName);
