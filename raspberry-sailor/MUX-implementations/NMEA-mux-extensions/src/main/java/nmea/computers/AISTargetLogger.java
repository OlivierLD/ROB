...
//	System.out.println(String.format("In AIS Target Computer, write method: %s", sentence));
		...
		if (StringParsers.validCheckSum(sentence)) {
			if (sentence.startsWith(AISParser.AIS_PREFIX)) {
				try {
					...
					if (aisRecord != null) {
						if (this.isVerbose()) {
							System.out.printf("(%s) %s received AIS MessType #%d, %s (verb: %s)\n", TimeUtil.getTimeStamp(), this.getClass().getName(), aisRecord.getMessageType(), sentence.trim(), this.verbose);
						}
						...
						if (this.isVerbose()) {
							System.out.printf(">>\tIn %s, AIS Mess null for %s\n", this.getClass().getName(), sentence.trim());
						}
					}
	...
	public static void main(String... args) {
		...
		try {
			System.out.println("JSON:" + mapper.writeValueAsString(cache));
		} catch (JsonProcessingException jpe) {
			jpe.printStackTrace();
		}
		System.exit(0);
