...
URL_OPTION_13c="http://localhost:${HTTP_PORT}/web/chartless.world.data.html"
...
URL_OPTION_13e="http://localhost:${HTTP_PORT}/web/index.html"
URL_OPTION_13e2="http://localhost:${HTTP_PORT}/web/chartless.world.data.html"
URL_OPTION_13f="http://localhost:${HTTP_PORT}/web/chartless.world.data.html"
...
while [[ "${GO}" == "true" ]]; do
	...
	if [[ "${SMALL_MENU}" == "Y" ]]; then
    echo -e "+-------------------------------------------------------------------------------------------------------------------+"
    echo -e "|               N A V   S E R V E R   -   L A U N C H E R  🚀                                                       |"
    echo -e "+-----------------------------------------------------------------------------------------+-------------------------+"
    echo -e "|  ${RED}0${NC}. Pure ES6 Celestial Context. External (and demanding on the browser). Requires Internet Connection.            |"
    echo -e "+-----------------------------------------------------------------------------------------+-------------------------+"
    echo -e "|  ${RED}1${NC}. Time simulated by a ZDA generator; HTTP Server, rich Web UI. Does not require a GPS.                          |"
    echo -e "|                     Full Nav Server Access (Including almanacs publication, Weather Wizard, etc)                  |"
    echo -e "|  ${RED}4${NC}. With GPS and NMEA data, waits for the RMC sentence to be active to begin logging                              |"
    echo -e "|                     (Check your GPS connection setting in nmea.mux.gps.properties file)                           |"
    echo -e "| ${RED}10${NC}. Full Nav Server Access. NMEA, Tides, Weather Wizard, Almanacs, etc. Data replay.                              |"
    echo -e "|                   - See or modify nmea.mux.properties for details.                                                |"
    echo -e "+-------------------------------------------------------------------------------------------------------------------+"
    echo -e "|  ${RED}S${NC}. Show NavServer process(es) ⚙️                                                                                 |"
    echo -e "+-------------------------------------------------------------------------------------------------------------------+"
    ...
    echo -e "| ${RED}12${NC}. With 2 input serial ports.                                                          | ${RED}13${NC}. AIS Tests (ais.exploratorium.edu data).                                             |"
    ...
    echo -e "|                                                                                         | ${RED}13e${NC}. GPS only, Chartless Map (La Trinite-Groix). GPS Only.                              |"
    echo -e "|                                                                                         | ${RED}13e2${NC}. GPS only, Chartless Map (Kernevel-La Trinite). GPS Only. Re-logs the data.        |"
    echo -e "|                                                                                         | ${RED}13f${NC}. GPS only, Chartless Map (Kernevel-La Trinite). GPS Only, no AIS.                   |"
    echo -e "|                                                                                         | ${RED}13g${NC}. GPS only, Chartless Map (La Trinite-Groix). GPS Only, AIS from OpenCPN/UDP         |"
    echo -e "|                                                                                         |                FogHorn collision/proximity detection.                                   |"
    ...
    # echo -e "+-----------------------------------------------------------------------------------------+-----------------------------------------------------------------------------------------+"
    echo -e "|  ${RED}S${NC}. Show NavServer process(es)                                                          | ${RED}SP${NC}. Show proxy process(es)                                                              |"
	...
	case "${option}" in
	  ...
	  H:*)   # Help on options below
	    ...
	    case "${HELP_ON}" in
	      ...
	      "13e")
	        ...
	        ;;
	      "13e2")
	        PROP_FILE=mux-configs/nmea.mux.replay.kernevel.la.trinite.2.yaml
	      	displayHelp ${HELP_ON} ${PROP_FILE} ${URL_OPTION_13e2}
	        ;;
	      "13f")
	        PROP_FILE=mux-configs/nmea.mux.replay.kernevel.la.trinite.yaml
	      	displayHelp ${HELP_ON} ${PROP_FILE} ${URL_OPTION_13f}
	        ;;
	      "13g")
	        PROP_FILE=mux-configs/nmea.mux.udp.ais.yaml
      ...
      if [[ "${option}" == "1" ]]; then
        if [[ "${LAUNCH_BROWSER}" == "Y" ]] || [[ "${LAUNCH_BROWSER}" == "y" ]]; then
          ...
          echo -en "Open NMEA Dashboard [1] (default) or Nav Menu [2] (all options menu) > "
          read REPLY
          if [[ "${REPLY}" == "2" ]]; then
