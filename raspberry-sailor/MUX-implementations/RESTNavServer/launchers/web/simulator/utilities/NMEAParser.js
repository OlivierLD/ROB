 ...
 *         UTC
 */
let generateRMC = (devicePrefix, date, lat, lng, sog, cog, decl) => {
  let rmc = devicePrefix + "RMC,";
  let UTCTime = date.getUTCHours().toFixed(0).padStart(2, '0') +
                date.getUTCMinutes().toFixed(0).padStart(2, '0') +
                date.getUTCSeconds().toFixed(0).padStart(2, '0');
  rmc += (UTCTime + ",");
  rmc += "A,";
  let deg = Math.trunc(Math.abs(lat));
  let min = 0.6 * ((Math.abs(lat) - deg) * 100.0);
  rmc += (deg.toFixed(0).padStart(2, '0') + (min.toFixed(3)));
  rmc += (lat < 0 ? ",S," : ",N,");

  deg = Math.trunc(Math.abs(lng));
  min = 0.6 * ((Math.abs(lng) - deg) * 100.0);
  rmc += (deg.toFixed(0).padStart(3, '0') + (min.toFixed(3)));
  rmc += (lng < 0 ? ",W," : ",E,");

  rmc += (sog.toFixed(1).padStart(5, '0') + ",");
  rmc += (cog.toFixed(1).padStart(5, '0') + ",");

  let UTCDate = date.getUTCDate().toFixed(0).padStart(2, '0') +
                (date.getUTCMonth() + 1).toFixed(0).padStart(2, '0') +
                (date.getUTCFullYear() % 100).toFixed(0).padStart(2, '0');
  rmc += (UTCDate + ",");
  rmc += (Math.abs(decl).toFixed(1).padStart(5, '0') + ",");
  rmc += (decl < 0 ? "W" : "E");

  let cs = checksum(rmc);
  rmc += ("*" + cs.toString(16).toUpperCase().padStart(2, '0'));
