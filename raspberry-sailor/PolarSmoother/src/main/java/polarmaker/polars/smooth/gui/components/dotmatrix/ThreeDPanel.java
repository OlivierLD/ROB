...
public class ThreeDPanel
	...
	private Vector<ThreeDPoint> speedPts = null;
	private Vector<ThreeDPoint> loggedPts = null;

	Model3D md = null;
	Model3D pts = null;
	Model3D loggedPoints = null;
	...
	public void setSpeedPts(Vector<ThreeDPoint> v) {
		this.speedPts = v;
	}
	public Vector<ThreeDPoint> getSpeedPts() {
		return this.speedPts;
	}

	public void setLoggedPoints(Vector<ThreeDPoint> logged) {
		this.loggedPts = logged;
	}
	public Vector<ThreeDPoint> getLoggedPoints() {
		return this.loggedPts;
	}
...
//    System.out.println("Running 3D for " + mdname);
			...
			if (this.loggedPts != null) {
				Model3D loggedModel = new Model3D(this.loggedPts);
				loggedPoints = loggedModel;
				loggedModel.findBB();
				loggedModel.compress();
			}
		} catch (Exception e) {
			md = null;
			message = e.toString();
...
//  System.out.println("Painting for " + mdname);
    ...
    */
		...
		if (loggedPoints != null) {
			loggedPoints.mat.unit();
			loggedPoints.mat.translate(-(md.xmin + md.xmax) / 2,
					-(md.ymin + md.ymax) / 2,
					-(md.zmin + md.zmax) / 2);
			loggedPoints.mat.mult(amat);
			loggedPoints.mat.scale(xfac, -xfac, 16 * xfac / getSize().width);
			loggedPoints.mat.translate(getSize().width / 2, getSize().height / 2, 8);
			loggedPoints.transformed = false;
			loggedPoints.paint(g, ThreeDPanel.Model3D.PLOT, Color.RED, Model3D.PLOT);
		}
		setPainted();
	}
	...
	final class Model3D {
		...
		Model3D(Vector<ThreeDPoint> v) throws Exception {
			this();
			// v
			int nb = 0;
		...
		private void paint(Graphics g, short opt) {
			paint(g, opt, null, ThreeDPanel.CIRC_OPT);
		}
		private void paint(Graphics g, short opt, Color extraPlotColor, short dOption) {
			drawingOption = dOption;
			...
			for (int i = 0; i < lim; i++) {
				...
				if (grey != lg) {
					lg = grey;
					g.setColor(opt == DRAW ? gr[grey] : (extraPlotColor != null) ? extraPlotColor : pointColor);
				}
			...
			//          g.setColor(Color.black);
					...
					g.setColor((extraPlotColor != null) ? extraPlotColor : pointColor);
				}
				if (opt == DRAW) {
					// Draw
