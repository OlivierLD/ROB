...
> this requires no satellite, no 5G antenna,...  nothing but a WiFi dongle. _**Veeeeery low**_ ⬇️ - if not null - carbon footprint, this may sound interesting to some.  
...
Some operations (like the build of the project 🙄...) would require an Internet connection. But definitely, we tried here to keep those requirements to a bare minimum. The runtime part can indeed happen offline (off-Internet).  
